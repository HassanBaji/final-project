...
type Params = Omit<
  AWS,
  'package' | 'provider' | 'functions' | 'resources' | 'plugins'
> & {
  ...
  plugins?: string[];
  package?: AWS['package'] | string[];
};

export default ({ functions, resources, plugins, ...rest }: Params): AWS =>
  merge<Partial<AWS>, Params>(
    {
      plugins: [
        ...new Set([
          'serverless-esbuild',
